...
const ROOM_ID = "railway_engineering";
...
export default function EvalPage() {
  ...
  const [activeId, setActiveId] = useState<string>(TEST_CASES[0].id);
  ...
  const runTest = useCallback(
    async (tc: TestCase) => {
      ...
      try {
        ...
        const conv = await createConversation(ROOM_ID, {
          project_config: {
  ...
  const runAssessment = useCallback(
    async (
      ...
      try {
        ...
        setAssessments((prev) => ({
          ...prev,
          [tc.id]: { status: "done", ...data },
        }));
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        setAssessments((prev) => ({
          ...prev,
          [tc.id]: { status: "error", error: msg },
        }));
      }
    },
    []
  ...
  }, [activeRun, activeTest, getToken, runAssessment]);
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-white border-b border-zinc-200 shrink-0">
        <div>
          ...
          <p className="text-xs text-zinc-500">
            Railway Engineering · {TEST_CASES.length} test cases
          </p>
        </div>
        <button
          onClick={runAll}
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 border-r border-zinc-200">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            {activeRun?.personaResponses &&
              activeRun.personaResponses.length > 0 && (
                <div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {activeAssessment?.status === "done" && (
              <div className="space-y-5">
                  ...
                  <div className="text-xs text-zinc-500 mb-2 space-y-1">
                    ...
                    {activeAssessment.routing?.unnecessary?.length ? (
                      <div className="text-orange-600">
                        Unnecessary:{" "}
                        {activeAssessment.routing.unnecessary.join(", ")}
                      </div>
                    ) : null}
                  </div>
                  {activeAssessment.routing && (
                ...
                {/* Persona quality */}
                {activeAssessment.persona_quality &&
                  activeAssessment.persona_quality.length > 0 && (
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400 mb-2">
                        Persona Quality
                      </p>
